...
Welcome to the in-depth documentation for **athanor** — personal LLM alchemy on Apple Silicon.

This directory contains technical guides covering environment prerequisites, runtime tuning, system architecture, terminal UI navigation, and diagnostic workflows.
...
| 🧪 **[Formulas & Model Tuning](formulas.md)** | Per-model overrides & inference optimization | Built-in formulas, custom formula library, Quantized KV Cache (`q8_0`/`q4_0`), Flash Attention, Speculative Decoding / MTP, and Reasoning Effort |
...
| 🖥️ **[Terminal User Interface (TUI)](tui.md)** | Interactive terminal workbench & monitoring | Layout, Downloads modal, Formula Editor, Telemetry modal, full-screen log navigation, mouse wheel capture, and tmux dev mode |
...
- **Pi-Agent Connection**: Athanor automatically updates `~/.pi/agent/models.json` with ingress aggregators (`athanor-mlx` and `athanor-llama`). See [Architecture: Pi-Agent Sync](architecture.md#pi-agent-catalog-synchronization).
- **Ingress Endpoints**: Front all models through `http://127.0.0.1:40879/v1` with on-demand model boot and stream-drain safety ([Architecture: The Ingress Server](architecture.md#the-ingress-server)).
...
- **Formula Basics**: Customize context size, sampling, and GPU layers per model ([Formulas: CLI Formula Management](formulas.md#cli-formula-management)).
- **Memory Pressure & Swap Thrashing**: Halve KV cache memory on high-context workloads using `q8_0` quantization and Flash Attention ([Formulas: KV Cache Quantization](formulas.md#kv-cache-quantization--flash-attention-llamacpp)).
- **Inference Speed**: Boost token generation 1.5–2× with Multi-Token Prediction or draft models ([Formulas: Speculative Decoding & MTP](formulas.md#speculative-decoding--mtp-llamacpp)).
- **Thinking Models**: Prevent runaway generations by constraining reasoning effort to `medium` or `low` ([Formulas: Reasoning Effort](formulas.md#reasoning-effort-llamacpp)).

### 🖥️ Monitoring & Workbench Operations
- **Interactive TUI**: Navigate installed models, view real-time CPU/RAM/tokens-per-second metrics, and tail logs ([TUI Guide](tui.md)).
- **Downloads Queue**: Manage active Hugging Face downloads and cancel running tasks ([TUI: Downloads Modal](tui.md#2-downloads-modal-d)).
- **Tuning UI**: Adjust simple compound knobs or granular flags live ([TUI: Formula Editor](tui.md#1-formula-editor-e)).
