...
describe("pi sync", () => {
  ...
  it("emits ingress-backed providers by default", async () => {
    ...
    expect(prov.baseUrl).toBe("http://127.0.0.1:40879/v1")
  ...
  it("emits per-runtime aggregator providers when router is enabled", async () => {
    vi.doMock("../config/index.js", async () => {
      ...
      return {
        ...
        loadConfig: () => ({
          ...real.DEFAULT_CONFIG,
          router: { enabled: true, host: "127.0.0.1", port: 40879 }
        })
      }
    })
    vi.doMock("../registry/index.js", () => ({
    ...
    expect(mlx.baseUrl).toBe("http://127.0.0.1:40879/v1")
    ...
    expect(llama.baseUrl).toBe("http://127.0.0.1:40879/v1")
