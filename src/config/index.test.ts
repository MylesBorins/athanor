...
describe("Config", () => {
  ...
  describe("loadConfig defaults", () => {
    it("returns default port range", () => {
      const c = loadConfig()
      expect(c.portRange.min).toBe(40880)
      expect(c.portRange.max).toBe(40979)
    })
    ...
    it("defaults router to enabled, bound to 127.0.0.1:40879", () => {
      const r = loadConfig().router
      expect(r.enabled).toBe(true)
      expect(r.host).toBe("127.0.0.1")
      expect(r.port).toBe(40879)
    ...
    it("exposes default mlx and llama knobs", () => {
      ...
      expect(c.mlx.promptCacheSize).toBe(65536)
      expect(c.mlx.contextWindow).toBe(65536)
      expect(c.mlx.maxTokens).toBe(4096)
      expect(c.llama.nGpuLayers).toBe(999)
      expect(c.llama.ctxSize).toBe(65536)
